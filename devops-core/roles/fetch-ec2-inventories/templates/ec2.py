...
class Ec2Inventory(object):
    ...
    def get_auth_error_message(self):
        ...
        if None in [os.environ.get('AWS_ACCESS_KEY'), os.environ.get('AWS_SECRET_ACCESS_KEY')]:
            errors.append(' - No AWS_ACCESS_KEY or AWS_SECRET_ACCESS_KEY environment vars found')
        else:
            errors.append(' - AWS_ACCESS_KEY and AWS_SECRET_ACCESS_KEY environment vars found but may not be correct')
...
def format_print(data):
    ...
    for key in keys:
        ...
        if (key.find(prefix) == 0):
            ...
            groups[groupName].extend(data[key])
    ...
    # output all groups and the all IPs under that group
    for key in groups.keys():
        print('[%s]' % key)
        for ip in groups[key]:
            hostStr  = ip.replace('_', '.') + ' ansible_ssh_private_key_file=~/.ssh/{{ keypair }}.pem'
            print(hostStr) # format 172_0_0_1 to 127.0.0.1

# Run the script
ec2inventory = Ec2Inventory()
data = yaml.safe_load(ec2inventory.data)
